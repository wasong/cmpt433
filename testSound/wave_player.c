...
//#define SOURCE_FILE "wave-files/Panic-Alarm-Sound-Effect.wav"
...
static pthread_t soundThreadId;


void sound_init(void){
    pthread_create(&soundThreadId, NULL, soundThread, NULL);
}
...
{
	/*I added */
	//Clipping on min/max volumes
	int volume;

	if (newVolume < 0) {
		volume = 0;
	} else if (newVolume > AUDIOMIXER_MAX_VOLUME){
