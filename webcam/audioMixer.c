...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	{
	  for (int i = 0; i < MAX_SOUND_BITES; ++i) {
	    //printf("Checking soundBites[%d]\n", i);
	    if (soundBites[i].pSound == NULL) {
	      soundBites[i].pSound = pSound;
	      soundBites[i].location = 0;

	      //printf("Queued soundBite at index %d\n", i);
	      
	      index = i;
	      i = MAX_SOUND_BITES + 1; // exit loop
	    }
	  }
	}
	...
	if (index == MAX_SOUND_BITES) {
	  printf("ERROR: no free sound slots available!\n");
	}

	//printf("Done queueing sound\n");
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
  ...
  //printf("Entering fillPlaybackBuffer()\n");
  memset(playbackBuffer, 0, (size * sizeof(*playbackBuffer)));

  playbackSound_t *currentSoundBite;
  ...
  for (int i = 0; i < MAX_SOUND_BITES; ++i) {
    if (soundBites[i].pSound != NULL) {
      ...
      //printf("soundBites[%d] is usable\n", i);
      currentSoundBite = &soundBites[i];
      currentLocation = currentSoundBite->location;
      playbackIndex = 0;
      
      while ((currentLocation < currentSoundBite->pSound->numSamples) && (playbackIndex < size)) {

	//printf("playbackIndex = %d\tsize = %d\n", playbackIndex, size);
	
	pData = (int) currentSoundBite->pSound->pData[currentLocation];
	data = ((int) playbackBuffer[playbackIndex]) + pData;
	...
	playbackBuffer[playbackIndex] = (short) data;

	++currentLocation;
	++playbackIndex;
      }

      currentSoundBite->location = currentLocation;
	  
      if (currentSoundBite->location >= currentSoundBite->pSound->numSamples)
	soundBites[i].pSound = NULL;
